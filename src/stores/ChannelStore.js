...
// Add 1 to the queue priority number on each download fetch to use the queue as LIFO  
let queuePriorityNumber = 0;
...
export default class ChannelStore {
  constructor({ network, channelName, peers }) {
    ...
    if (window.prefilledChannelPeers && window.prefilledChannelPeers[channelName]) {
      this.peers = window.prefilledChannelPeers[channelName];
    } else {
  ...
  get uniqueUsers() {
    ...
    for (let i = 0; i < this.peers.length; i++) {
      if (uniqueNameList.indexOf(this.peers[i].userProfile.name) == -1) {
        uniqueUserList.push(this.peers[i]);
        uniqueNameList.push(this.peers[i].userProfile.name);
      }
    }
  ...
  _removeEntries(entries) {
    if (!(entries instanceof Array)) return
    entries.forEach(e => (delete this.entriesMap[e.hash]))
  }
  ...
  @action
  _updatePeers(peers) {
    this.channelPeersInitialized = true;
    this.peers = peers
  }
  ...
  _addPeers(peers) {
    for (let i = 0; i < peers.length; i++) {
      let peerFound = false;
      for (let y = 0; y < this.peers.length; y++) {
        if (peers[i].peerID == this.peers[y].peerID) {
          peerFound = true;
          continue;
        }
      }
      if (!peerFound) {
        this.peers.push(peers[i]);
      }
    }
  }

  @action
  _removePeers(peerIDs) {
    for (let i = 0; i < peerIDs.length; i++) {
      for (let y = 0; y < this.peers.length; y++) {
        if (peerIDs[i] == this.peers[y].peerID) {
          this.peers.splice(y, 1);
          y--;
        }
      }
    }
  ...
  _onEntry(entry) {
    ...
    if (this.network.settingsStore.networkSettings.blockedIdentityIDs.indexOf(entry.identity.id) > -1) {
      return
    }


    // Fixes issue where user submits a future date and message will not move from the bottom of the screen
    if (entry.payload.value.meta.ts > Date.now()) {
      entry.payload.value.meta.ts = Date.now();
    }
  ...
  // Called when before an official orbitdb write has been finalized
  _onTempWrite(entry) {
    this._markEntryAsRead(entry)
    this._updateEntries([entry])
  }

  // Called after orbitdb write has been finalized
  _onTempDelete(entry) {
  ...
  _onPeersAdd(peers) {
    let checkExist = setInterval(() => {
      if (this.channelPeersInitialized) {
        clearInterval(checkExist);
        this._addPeers(peers)
      }
    }, 100);
  }

  _onPeersRemove(peerIDs) {
    this._removePeers(peerIDs)
  }
  ...
  async loadFile(hash, onStreamEvent) {
    return new Promise((resolve, reject) => {
      fileQueue.add(async () => {
        try {
          const response = await this.network.workerProxy.getFile({ hash, timeout: 20 * 1000 },
            onStreamEvent
          )
