...
export default class SettingsStore {
  constructor(rootStore) {
    ...
    // Wait for uiStore to load
    let checkExist = setInterval(() => {
      if (rootStore.uiStore) {
        clearInterval(checkExist);

        // Only query server if suggested moderation is found
        if (rootStore.uiStore.useSuggestedModeration) {
          const moderatorURL = defaultSettingsOptions.bootstrapNodes[0].split("/")[2];
          fetch(`https://${moderatorURL}/v0/channels/allUserBans`)
            .then((response) => {
              return response.json();
            })
            .then((banInfo) => {
              this._addBlockedIdentityID(banInfo.userBanList);
            }).catch((error) => {
              console.error(error);
            });
        }
  ...
  @computed
  get _returnBlockedIdentityIDs() {
    return this.networkSettings.blockedIdentityIDs
  }
  ...
  @action
  _addBlockedIdentityID(identityIDList) {
    for (let i = 0; i < identityIDList.length; i++) {
      this.networkSettings.blockedIdentityIDs.push(identityIDList[i]);
    }
    this._saveNetworkSettings();
  }

  @action
  _removeBlockedIdentityID(identityIDList) {
    for (let i = 0; i < identityIDList.length; i++) {
      const indexCheck = this.networkSettings.blockedIdentityIDs.indexOf(identityIDList[i]);
      this.networkSettings.blockedIdentityIDs.splice(indexCheck, 1);
    }
  ...
  @action
  _clearBlockedIdentityID() {
    this.networkSettings.blockedIdentityIDs = [];
    this._saveNetworkSettings();
  }
