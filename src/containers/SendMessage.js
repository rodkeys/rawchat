...
// Do not let user send more than 5 messages a minute to prevent spam
let submittedMessages = [];

setInterval(() => {
    submittedMessages.splice(0, 1)
}, 10000)
...
function SendMessage({ channelName, onSendMessage, theme, useEmojis, emojiSet, disabled }) {
  ...
  const [peerPickerActive, setPeerPickerActive] = React.useState(false)
  ...
  const handleInputSubmit = React.useCallback(
    e => {
      e.preventDefault()
      if (submittedMessages.length > 5) {
        alert("You are sending too many messages. Please wait a little bit before sending more");
        return;
      }
      submittedMessages.push(inputRef.current.value)
  ...
  const handleInputChange = React.useCallback(() => {
    ...
    const peerPickerActive = lastWord.startsWith('@')
    ...
    setPeerPickerActive(peerPickerActive)
  }, [useEmojis])
  ...
  const handleEmojiChange = React.useCallback(
    ...
    const handlePeerChange = React.useCallback(
    (peer, done = false) => {
      if (!peer || !inputRef.current) return
      const i = inputValue.lastIndexOf('@')
      const text = inputValue.slice(0, (i + 1)) + peer.userProfile.name

      setInputValue(text + " ")
      setPeerPickerActive(!done)
  ...
  React.useLayoutEffect(() => {}, [channelName, disabled, focusInput])

  function getEmojiPickerStyle(emojiSize) {
    return {
      maxWidth: (emojiSize + 2) * 10, // 2 * 1px padding or border
      bottom: inputRef.current ? inputRef.current.offsetHeight + 22 : '47px'
    }
  }

    function getPeerPickerStyle(emojiSize) {
    return {
      bottom: inputRef.current ? inputRef.current.offsetHeight + 22 : '47px'
    }
  }
;
  ...
  const pickerPeerSize = 24;
  const maxNumberOfPeers = 5;
  return (
      ...
      <form onSubmit={handleInputSubmit}>
        ...
        {peerPickerActive ? (
          <PeerPicker
            ...
            style={getPeerPickerStyle(pickerPeerSize)}
          />
        ) : null}
