...
function LoginView() {
  ...
  React.useEffect(() => {
    ...
    if (splitPathName[1] == "channel") {
      let channelName = splitPathName[2];
      setDefaultChannels([
        [channelName, []]
      ])

      fetch(`https://${moderatorURL}/v0/channel/${channelName}`)
        .then((response) => {
          return response.json();
        })
        .then((channelInfo) => {
          setDefaultChannels([
            [channelInfo[0], channelInfo[1]]
          ]);
          window.prefilledChannelPeers[channelInfo[0]] = channelInfo[1];
          let checkExist = setInterval(() => {
            // If IPFS/Orbit is connected then join relevant channels
            if (networkStore.isOnline) {
              clearInterval(checkExist);
              networkStore.joinChannel(channelInfo[0], channelInfo[1]);
            };
          }, 100);
        }).catch((error) => {
          console.error(error);
      ...
      uiStore.setTitle('Login | Raw.chat')
      uiStore.closeControlPanel()

      fetch(`https://${moderatorURL}/v0/channels/default/10`)
        .then((response) => {
          return response.json();
        })
        .then((channels) => {
          ...
          let checkExist = setInterval(() => {
            ...
            if (networkStore.isOnline) {
              clearInterval(checkExist);
              for (let i = 0; i < channels.length; i++) {
                networkStore.joinChannel(channels[i][0], channels[i][1]);
              };
            };
          }, 100);
        }).catch((error) => {
    ...
    !sessionStore.isAuthenticated ? (
      <div className='LoginView'>
        <h1 className='loginHeaderAnimation'>
          <div>Join a room</div> 
          <div>and chat now</div>
        </h1>

        <h2 className='loginHeaderAnimation loginMainSubHeader fadeInAnimation'> 
